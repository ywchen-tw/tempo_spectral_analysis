...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--rerun-fitting",
        action="store_true",
        help="Ignore cached fitting inputs and rerun WP1–WP6 from scratch.",
    )
    return p.parse_args()
...
def main() -> int:
    ...
    fitting_cache_dir = os.path.join(ROOT, "outputs", "fitting_cache",   run_tag or "default")
    ...
    # chunk_tag is fully determined by args; compute early so cache paths can use it
    ...
    fitting_inputs_npz = os.path.join(fitting_cache_dir, f"fitting_inputs_{chunk_tag}.npz")
    fitting_pix_csv    = os.path.join(fitting_cache_dir, f"pixel_table_{chunk_tag}.csv")

    spectral_fitting_order = int(args.spectral_fit_order)

    _from_cache = (
        not args.rerun_fitting
        and os.path.exists(fitting_inputs_npz)
        and os.path.exists(fitting_pix_csv)
    )
    if _from_cache:
        print(f"Loading cached fitting inputs from: {fitting_inputs_npz}")
        _cached = np.load(fitting_inputs_npz, allow_pickle=False)
        tau_fit = {
            "pixel_index":   _cached["pixel_index"],
            "wavelength_nm": _cached["wavelength_nm"],
            "tau_eff":       _cached["tau_eff"],
            "ring_basis":    _cached["ring_basis"],
        }
        tau_mode2    = _cached["tau_mode2"]
        obs_ln_T_fit = _cached["obs_ln_T_fit"]
        pix = pd.read_csv(fitting_pix_csv)
    else:
        if args.rerun_fitting:
            print("--rerun-fitting set: rerunning WP1–WP6 from scratch.")
        ...
        # ── WP7 assembly: observed lnT + per-pixel tau stacks ─────────────────
        ...
        # ── Save fitting inputs cache ──────────────────────────────────────────
        os.makedirs(fitting_cache_dir, exist_ok=True)
        np.savez_compressed(
            fitting_inputs_npz,
            pixel_index   = tau_fit["pixel_index"],
            wavelength_nm = tau_fit["wavelength_nm"],
            tau_eff       = tau_fit["tau_eff"],
            ring_basis    = tau_fit["ring_basis"],
            tau_mode2     = tau_mode2,
            obs_ln_T_fit  = obs_ln_T_fit,
        )
        pix.to_csv(fitting_pix_csv, index=False)
        print(f"Saved fitting inputs cache: {fitting_inputs_npz}")
